accRun <- function(featureData,
  ...
  featureData <- featureData[featureData$nRows > 3,]
  
  ###remove inf and replace with NA
  featureData <- do.call(data.frame,lapply(featureData, function(x) replace(x, is.infinite(x),NA)))
  
  if(Dummies){
    ##### add in the seal specific features
    sealCharacteristic <- read.csv("animal_details2.csv")
    
    featureData <- merge(featureData, sealCharacteristic, by = "SealName")
    
    featureData$Place <- factor(featureData$Place)           
    featureData$Gender <- factor(featureData$Gender)           
    featureData$species <- factor(featureData$species)           
    featureData$harsness <- factor(featureData$harsness)  
  }
  else{
    featureData$Place <- NULL
  }
  ...
  trainData <- sampledData[, !(names(sampledData) %in% c("FileDate", "SealName", "nRows", "X", "Behaviour"))]
  ...
  switch(Model,
         ...
         "SVM" = {
           source("r/models/parallelCVSvm.r")
         ...
         "RLR"={
           source("r/models/parallelCvRlr.r")
         ...
         "GBM" = {
           source("r/models/parallelCVGBM.r")
  ...
  if(SAVE){
    if(Model == "SVM"){
      fName <- paste0(Model, " Model with ",Kernel," kernel, ", K, "-fold CV", ".RData")
    }else{
      fName <- paste0(Model, " Model, ", K, "-fold CV", ".RData")
    }
    cat("##### saving", fName, "\n")
    
    mainDir <- "output"
    if(codeTest){
      subDir <- "outputDirectory/test"
    }
    else{
