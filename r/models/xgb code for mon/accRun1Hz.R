accRun <- function(featureData,
  ...
  for(i in 1:length(uEventIds)){
    ...
    if(nr>classMin){
      sampleIdx <-sample.int(n = nr, size = classMin)
  ...
  ##Up sample the small classes
  sampledData <- upSample(sampledData[,-ncol(sampledData)],
                          factor(sampledData$EventIds))
  colnames(sampledData)[57]<-"EventIds"
  ...
  switch(Model,
         "XGB" = {
           ...
           finalPreds <- outputData$testDataSplit$predsTest
             ...
             fittnessCV <- outputData$paramList
         },
         "rPart" = {
  ...
  if(SAVE){
    ...
    #mainDir <- getwd()
    if(codeTest){
      subDir <- "output/outputDirectory/test/"
    }
    else{
      subDir <- "output/outputDirectory/"
    }
