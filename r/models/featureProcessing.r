featureProcessing <- function(epochs = 5, fileLoc){
  library(e1071)  
 ...
 #    setwd("C:/Users/admin/Google Drive/Behaviour accelerometry")
  ...
  for(j in 1:length(SealNames)){
    ...
    for(k in 1:nFileNames){
      ...
      for(i in 1:length(Events)){
        ...
        ODBA.auc   <- auc(dat$time,   dat$ODBA) 
        VeDBA.auc   <- auc(dat$time,   dat$VeDBA)  
  ...
  save(outputData, file=paste0("output/1Hz/outputData_",epochs,".RData"))
