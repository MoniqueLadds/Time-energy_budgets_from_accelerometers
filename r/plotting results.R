...
load("output/proportions_nofeature.RData")
...
chi.sq<- proportions %>% group_by(season,Var.1,Var.2) %>%
  dplyr::summarise(N = sum(value),
                   prop = N/12341322)
write.csv(chi.sq,"output/summaries/chi_square_table.csv")
...
write.csv(prosum,"output/summaries/place_event.csv")
...
write.csv(proportions,"output/summaries/proportions.csv")
...
load("output/all_data_nofeature.RData")
...
all_data$id<-as.factor(paste0(all_data$place,"_",substr(all_data$Seal,1,6)))
  ...
  sealPlot$totKj<-sealPlot$totEE/1000*24*5*4.186
...
##add DEE kg to the data
sealData$totKj<-sealData$EE_day/1000*24*5*4.186
...
write.csv(sealSumm,"output/summaries/Table3.csv")
